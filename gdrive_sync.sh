...
base_dir=$( python json_parser.py --config $CONFIG --key base_path )
src_dir=$DIR/${base_dir}
dest_dir=$( python json_parser.py --config $CONFIG --key dest_path )
echo "from $src_dir to $dest_dir"

py=$( python json_parser.py --config $CONFIG --key python )
